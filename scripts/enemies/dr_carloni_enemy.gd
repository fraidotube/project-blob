...
func _resume_combat_state() -> void:
	if state == State.DEAD:
		return

	# Se il martello è fisicamente in mano, la AI deve tornare SEMPRE a HELD.
	if held_hammer != null and held_hammer.visible and flying_hammer == null:
		state = State.HELD
		held_timer = minf(held_timer, 0.35)
		return

	# Se il martello è ancora in volo, resta nella fase senza martello.
	if flying_hammer != null:
...
func _process_chase() -> void:
	# FAILSAFE IMPORTANTISSIMO:
	# CHASE non deve mai "dimenticare" che Carloni ha il martello in mano.
	if held_hammer != null and held_hammer.visible and flying_hammer == null:
		state = State.HELD
		return
