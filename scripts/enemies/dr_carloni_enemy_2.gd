...
@export var melee_range: float = 2.25
...
func _process_held(delta: float) -> void:
	...
	# Se il player è vicino e Carloni ha il martello, prima il corpo a corpo.
	if distance <= melee_range and melee_cooldown <= 0.0 and _has_line_of_sight_to_player():
		if _can_use_combo():
			_start_combo()
		else:
			_start_melee()
	...
	# Se il player è abbastanza lontano, continua a poter lanciare il martello.
	if held_timer <= 0.0:
		if distance >= hammer_throw_min_distance and distance <= hammer_throw_max_distance and _has_line_of_sight_to_player():
			_start_throw()
			return
